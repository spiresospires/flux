...
**Validated on a genuinely cold cache — the commit of this very change.** The pre-commit hook run for commit `8002791` logged **`prewarm: test environment warmed in 104278ms`** and then passed **180/180 with `environment` at 12.14s** (total 92.08s). That is the decisive evidence: 104 seconds of cold module reading was absorbed *outside* the timed window, where before it would have landed inside a 60s ceiling and killed the worker. Unlike §30's and §33's warm-run tallies, this is a positive demonstration under the exact condition that was breaking commits. Standing invariant (`docs/quality-gates.md` Stage C): *prewarm may be arbitrarily large; reported `environment` must stay well under 60s* — observed 3.57s warm, 12.14s cold; treat >25s as the warning sign.
...
## 37. Outlook-Style Row Navigation in Documents (2026-08-13)

**Ask:** make the document grid behave like new Outlook's message list + reading pane — a roving preview cursor and a checkbox selection that never touch each other, with the properties sidebar collapsing once more than one row is checked.

**Half of it already existed, and the half that existed was wrong in one specific way.** `activeDocId`, `selectedDocumentIds` and `selectionAnchorRef` were already there, focus already stayed on the container, `scrollIntoView({block:'nearest'})` was already wired, and Arrow/Shift+Arrow/Space/Ctrl+A already worked. But `handleRowActivate` toggled the checkbox **and** moved the cursor on a plain row click, and the sidebar was not cursor-driven at all: `panelData` was snapshot state pushed in by whichever affordance was clicked (reference link, action menu, card). Those two facts are what the ask inverts.

**The sidebar is now derived, not pushed.** `panelData` is a `useMemo` over the active row; `setPanelData` is gone. `openDocumentPanel`/`closeDocumentPanel` became `previewDocument`/`dismissDetailPanel`, and all five former call-sites now just move the cursor. One source of truth replaced three writers.

**Deep links no longer tick a checkbox.** The old effect seeded `selectedDocumentIds` from `?doc=`, so arriving from enterprise search left you holding a one-row bulk selection you never made — the exact conflation the two-axis model exists to prevent. It now seeds the cursor only.

**The `?doc=` echo problem.** The param is two-way now (the cursor writes it, a deep link reads it). The seeding effect cannot simply compare against `activeDocId`: the URL is written from the *debounced* cursor, so mid-arrow the two legitimately disagree, and seeding on that difference drags the cursor backwards onto the row the user just left. `urlDocWrittenRef` records what we wrote so the effect can recognise its own echo.

**Debounce is not decoration.** `toDocumentDetail` runs `buildJourney` + `buildVersionStack` per subject, and the G06 single-document query would otherwise fire once per row skated past. Two mitigations: a 160 ms trailing debounce on the panel's subject, and the fetch gated on the *row lookup failing* rather than on a doc being previewed — arrowing through loaded rows never touches the network.

**Collapse animates the wrapper, not the mount.** `DetailSlidePanel`'s own `AnimatePresence` lives inside the width wrapper, so gating the wrapper on the checked count would have unmounted the presence boundary with it: the exit never plays and the grid snaps wider in one frame. The wrapper animates `width: 0 ↔ panelWidth` instead, with a clipping box inside it and a fixed-width inner div so the panel's text doesn't re-wrap while it closes. `PanelResizeHandle` sits 16 px outside the panel's left edge, so it cannot live in the clipping box — it is dropped outright while collapsed rather than left floating over the grid.

**ARIA moved onto the table, not the scroll wrapper.** `role="grid"` + `tabIndex` + `aria-activedescendant` all have to sit on the same element (the one that holds both the composite role and the focus), so they went on `<table>`; the scroll div stayed a plain scroll wrapper, which `scrollRowIntoView` still targets. Rows carry `id="docrow-<docId>"` and `aria-selected` = **checked**, not active. A `focus-visible` ring was added to the table because tabbing in before any row is active would otherwise show nothing at all.

**Two real defects found by testing in the browser, both fixed:**

1. `scrollIntoView({block:'nearest'})` is **not enough** with a sticky header. Measured after Home: active row `top: 165`, `thead` bottom `189` — the row sat 24 px underneath the column headers and read as having vanished, while 'nearest' considered it perfectly visible. `scrollRowIntoView` now measures the overlap and nudges `scrollTop`. Re-measured: gap 0, `scrollTop` 0.
2. The live-region collapse message originally carried the checked count. It announces only on the collapse/expand *transition*, so extending a range from 2 to 5 rows left "2 documents selected" sitting in the region — stale text a screen reader can re-read. The count came out of the message (the bulk bar carries it visually, and each row's `aria-selected` carries its own state); re-announcing on every Shift+Arrow to keep a number current would be noise.

**Bulk action bar is new.** Criterion 6 said "existing component", but nothing acted on `selectedDocumentIds` — it drove the row tint and the header tri-state checkbox and nothing else. Clipboard / Briefcase / Export-selected are wired to what exists; Download and Transmit are rendered **disabled** with `[TODO-ENG]` markers (G07 content retrieval and transmittal creation don't exist in the prototype). Disabled rather than absent: they are the first two actions a user reaches for, and a bar that silently omits them reads as "not supported" instead of "not built yet".

**Open design question, deliberately not answered:** should checking exactly one row also make it active, so ticking a single box previews it? Built as specified — no, the axes stay independent at count 1 — but this needs confirming with design rather than assuming.

**Self-inflicted damage worth recording:** a `Get-Content | Set-Content` round-trip in PowerShell 5.1 to add a few Tailwind classes read the file as ANSI and rewrote it as UTF-8, mojibaking all 84 non-ASCII sequences (every em dash → `a-EUR"`) and adding a BOM. Reversed losslessly by re-encoding the string to cp1252 and decoding those bytes as UTF-8 (verified: zero `?` placeholders introduced, 0 mojibake after, 74 em dashes and 231 box-drawing chars restored). **Never route source files through `Get-Content`/`Set-Content` for a text substitution** — use the editor. This is the same class of bug the filename/encoding rules in CLAUDE.md exist to prevent.

**Verified in the browser** (dev server, 1440x900): row-body click sets active with 0 checkboxes checked; checkbox click checks without moving the cursor or changing the sidebar; ArrowDown x3 moves 3 rows with nothing checked; Home/End land on first/last loaded row; Ctrl+click and Shift+click both check without moving the cursor (5-row contiguous range confirmed exact); 2 checked collapses the panel to `width: 0, opacity: 0` with the announcement firing; arrowing continues to work while collapsed; unchecking to 1 re-expands showing the **active** row (not the checked one); Enter opens the framed viewer and no-ops on a placeholder; panel X hides the panel while `activeDocId` survives and the next arrow reopens it; select-all takes 40 rows without moving the cursor; bulk Clipboard added exactly the 5 checked docs (localStorage snapshotted and restored afterwards, so no residue).

**Not verified by a real keypress:** Space. The preview harness cannot deliver a space key to the grid — `key: "Space"` arrives with an empty `key`/`code`, and the `type` action produces no keydown at all. Exercised with a dispatched `KeyboardEvent({key:' '})` instead, which does reach the real React handler (proved by dispatching ArrowUp the same way and watching the cursor move, and by `defaultPrevented === true` on the dispatched space). An early read said Space "didn't work" — that was two dispatches toggling the same row on then off, not a bug.

**Two more defects caught in review, after the first round of browser verification:**

3. **Enter hijacked every focused control in the grid.** The handler bubbles from the `<table>`, so it runs *before* the browser activates a focused button — and the Enter branch called `preventDefault()` with no target guard. Enter on the row ⋯ menu, the reference link, the Flint/clipboard buttons, a column sort header or the select-all checkbox opened the framed viewer instead of activating the control. A regression this change introduced (Enter previously had no handler at all) and squarely against the no-regressions criterion. Fixed with the same `target.closest('button, a')` guard the Space branch already had. Verified both directions: Enter on a row's ⋯ button no longer opens the viewer, Enter on the grid still does.
4. **`aria-activedescendant` could dangle.** The cursor survives list changes, so a column filter excluding the active row — or collapsing its group — left the grid pointing at an id absent from the DOM. Now only emitted while `navigableDocs` still contains it.

**Known limitation, not fixed — the acceptance list's "Tab into grid, arrow to a row, Tab to its checkbox" flow does not work literally.** From the focused table, Tab follows document order: the select-all checkbox, then every column sort button, then row 1's controls. Arrow to row 20 and Tab lands nowhere near row 20's checkbox. The *intent* holds — Space on the grid toggles the active row, so a keyboard-only user can check rows and build a range with no mouse (verified) — but making the literal flow work means `tabIndex={-1}` across every header control and row button, i.e. the full ARIA grid focus-management pattern. That is a larger change than this task implies and is left as a decision.
